...
```{r}
#List kids with a MLU > 2.7
#Find kids who have a MLU > 2.7
above2.7 = filter(data, CHI_MLU > 2.7)
#Write out the specific kids
unique(above2.7$SUBJ)


#List kids with MLU < 1.5 at the first visit
below15 = filter(data[data$VISIT == 1,], CHI_MLU < 1.5)
...
#As suspected it just adds it more times and adds a number to the variable name.
...
data = mutate(f, mean_all = mean(f$tokens_CHI))
